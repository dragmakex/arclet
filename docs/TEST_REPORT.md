...
| `bun run test` | PASS | 52 offline unit assertions across 13 files, including signature/origin binding, funding-sender binding, recurring scheduling, decision-time Graph freshness, pause/lease fencing, withdrawal gas-buffer guards, and desk chart/data normalization |
...
| `bun run test:e2e` | PASS | 1 Chromium unconfigured-desk test: root forwards to the desk, empty ledger/chart remain explicit, and unavailable actions stay disabled |
| `bun run build` | PASS | Next.js production build and route generation, including authenticated `/api/desk` and `/api/portfolio-history` read routes |
