...
export async function compileStrategy(input: { model: LanguageModel; instruction: string; marketId: string; allowedMarketSummary: string }) {
  ...
  const result = await generateObject({ model: input.model, schema: compileResultSchema, system: SYSTEM, prompt: JSON.stringify({ instruction: input.instruction, requestedMarketId: input.marketId, allowedMarketSummary: input.allowedMarketSummary }), maxRetries: 1 });
